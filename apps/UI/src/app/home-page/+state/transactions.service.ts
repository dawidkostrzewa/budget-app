...
import { CategoryAmountSummary } from './category.model';
...
export class TransactionsService {
    ...
    getTransactionsAmountByCategory(catId: number) {
        return this.transactionFacade
            ...
            .pipe(
                switchMap((amount) =>
                    this.transactionFacade.getCategoryById(catId).pipe(map((category) => ({ amount, category })))
                )
            );
    }
        ...
        return combineLatest([
            ...
            map(([transactions, categories, mainCategories]) =>
                transactions.map((t: Transaction) => ({
                    ...
                    mainCategoryName:
                        mainCategories.find(
                            (mC) => mC.id === categories.find((c) => c.id === t.categoryId)?.mainCategoryId
                        )?.name || 'Not found'
                }))
            )
        );
